...
class FiberHome extends Controller
...
function Connect($host, $port = 3337) {
...
//print_r($list);
    if ((strpos($list[4], 'ENDESC=resource does not exist') > 0) or ( strpos($list[4], 'ENDESC=invalid parameter format') > 0)) {
        return $list[4];
    } else {
        $tmp = explode("=", $list[4]);
...
public function ONU_WANMacAddr($PONID, $ONUID, $ONUTYPE = 'MAC') {
    return $this->ReturnRecords($this->cmd("LST-PORTMACADDRESS::OLTID={$this->OLT},PONID={$PONID},ONUID={$ONUID},ONUIDTYPE={$ONUTYPE},PORTID=NA-NA-NA-1,VLAN=200:CTAG::;"));
}
...
public function ONU_Add($PONID, $AUTHTYPE, $ONUID, $NAME, $ONUTYPE = 'AN5506-04-B2') {
    $ONUID = trim($ONUID);
...
public function ONU_LAN_VLAN_ADD($PONID, $ONUID, $ONUTYPE = 'MAC', $ONUPORT, $configs) {
    /*
     * SVLAN=
     * CVLAN=
     * UV=
    ...
    $CONFIG_ = substr($CONFIG_, 1);
    $list = $this->cmd("CFG-LANPORTVLAN::OLTID={$this->OLT},PONID={$PONID},ONUID={$ONUID},ONUIDTYPE={$ONUTYPE},ONUPORT={$ONUPORT}:CTAG::{$CONFIG_};");
    if ($this->CMD_OK($list)) {
        return true;
    } else {
...
public function ONU_LAN_VLAN_DEL($PONID, $ONUID, $ONUTYPE = 'MAC', $ONUPORT, $configs) {
    ...
    $CONFIG_ = substr($CONFIG_, 1);
    $list = $this->cmd("DEL-LANPORTVLAN::OLTID={$this->OLT},PONID={$PONID},ONUID={$ONUID},ONUIDTYPE={$ONUTYPE},ONUPORT={$ONUPORT}:CTAG::{$CONFIG_};");
    if ($this->CMD_OK($list)) {
        return true;
    } else {
        echo "DEL-LANPORTVLAN::OLTID={$this->OLT},PONID={$PONID},ONUID={$ONUID},ONUIDTYPE={$ONUTYPE},ONUPORT={$ONUPORT}:CTAG::{$CONFIG_};\n";
        print_r($list);
        return false;
    }
}
...
public function ONU_Del($PONID, $ONUID, $ONUTYPE = 'MAC') {
